...
3.填写完成后，单击**确定**，该数据应用以列表的形式置于数据应用列表中。
### 新建工作流
工作流是数据应用的具体应用方式，用户可以按照实际应用需求创建工作流。创建工作流需完成以下步骤：
1. 在**数据应用**列表中找到已创建的数据应用。
2. 单击该数据应用右侧的**工作流**。
3. 在**数据模块**下选择需要的数据模块并拖拽到右侧画布中。
4. 在**分析模块**下选择需要的模块并拖拽到右侧画布中。
5. 光标移动到任意数据模块的小空心圆点上按住鼠标左键并把连线连接到选中的分析模块上。
6. 单击**保存**，保存创建的工作流。
...
### 编辑数据应用
在已创建的数据应用中用户可以随时对这些数据应用进行编辑。编辑数据应用需完成以下步骤：
...
**注意**：如果运行模式选择**无**则工作流将立刻运行，如果运行模式选择**稍后运行**则会弹出一个选框让你选择开始运行的日期；如果运行模式选择**定期运行**则会出现开始日期、定期模式以及过期时间让你选择。
